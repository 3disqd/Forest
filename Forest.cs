...
namespace Forest
{
	...
	class FunkyForest : IForest
	{
        ...
        public bool TryMove(string creatureName, Point vector)
        {
            Creature chosenOne;
            var exists = creatures.TryGetValue(creatureName, out chosenOne);
            if (exists)
        ...
        public void Auto(string creatureName, Point destination)
        {   
            //алгоритм требует доработки, но принцип понятен
            while (!(destination.Equals(creaturePosition[creatures[creatureName]])))
            {   
                while(TryMove(creatureName, new Point(0, 1)))
                {
                    Move(creatureName, new Point(0, 1));
                    Console.ReadKey();
                    continue;
                }
                while (TryMove(creatureName, new Point(1, 0)))
                {
                    Move(creatureName, new Point(1, 0));
                    Console.ReadKey();
                    continue;
                }             
                while (TryMove(creatureName, new Point(0, -1)))
